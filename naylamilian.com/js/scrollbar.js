	...
	*/
	...
	self.$el.addClass(prefix + 'host').wrapInner('<div class="' + prefix + 'viewport"><div class="' + prefix + 'container" /></div>').append(self.$scrollbar);
	self.$viewport = self.$el.find('> .' + prefix + 'viewport');
	self.$container = self.$viewport.find('> .' + prefix + 'container');
	self.$el.on('mousewheel', function (event, delta, deltaX, deltaY) {
	...
	if (appendContent) {
	$panelContent.prepend('<div class="">' + length + '</div>');
	} else {
