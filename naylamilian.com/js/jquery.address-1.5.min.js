	...
	*
	* Date: 2012-08-17 16:32:52 -0700 (Fri, 17 Aug 2012)
	*/
	(function (c) {
	c.address = function () {
	var r = function (a) {
	...
	e = _old;
	if (D())
	y.popState({}, "", d.state.replace(/\/$/, "") + (e === "" ? "/" : e));
	else {
	w = n;
	if (A)
	if (d.history)
	...
	b = (i.pathname + i.search + (c.address && !D() ? c.address.value() : "")).replace(/\/\//, "/").replace(/^\/$/, "");
	if (c.isFunction(a))
	a(b);
	...
	M = function () {
	var a = V() + ":" + k + ";document.open();document.writeln('<html><head><title>" + o.title.replace(/\'/g, "\\'") + "</title><script>var " + B + ' = "' + encodeURIComponent(u()).replace(/\'/g, "\\'") + (o.domain != i.hostname ? '";document.domain="' + o.domain : "") + "\";<\/script></head></html>');document.close();";
	if (E < 7)
	l.src = a;
	else
	...
	v(b, "paddingBottom")) + "px " + (v(b, "marginLeft") + v(b, "paddingLeft")) + 'px;" />').parent().wrap('<div id="' + B + '" style="height:100%;overflow:auto;position:relative;' + (A && !window.statusbar.visible ? "resize:both;" : "") + '" />');
	c("html, body").css({
	height : "100%",
	margin : 0,
	padding : 0,
	...
	A && c('<style type="text/css" />').appendTo("head").text("#" + B + "::-webkit-resizer { background-color: #fff; }")
	}
	if (z && !F) {
	...
	_old = e;
	if (z) {
	E = parseFloat(C.substr(C.indexOf("MSIE") + 4));
	if (o.documentMode && o.documentMode != E)
	E = o.documentMode != 8 ? 7 : 8;
	...
	G(n);
	if (D())
	y[d.history ? "pushState" : "replaceState"]({}, "", d.state.replace(/\/$/, "") + (e === "" ? "/" : e));
	else {
	w = n;
	if (A)
		if (d.history)
